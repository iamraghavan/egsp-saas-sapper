<script>
  ...
  onMount(async () => {
    ...
    const response = await fetch('https://nirvaagam-backend.onrender.com/as-executive-users');
                          ...
                          <tbody>
                              {#each tickets as ticket, index (ticket.id)}
                                <tr>
                                  ...
                                  <td>
                                    <!-- Print icon -->
                                    <button class="btn btn-primary" on:click={() => handleStatus(ticket.ticket_id)} class:disabled={ticket.status === 'closed'}>
